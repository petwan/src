...
from app.api.v1.auth.service import WechatAuthService
...
async def phone_login(
    ...
    auth_service: WechatAuthService = Depends(
        get_wechat_auth_service
    ),  # 这里用 WechatAuthService 暂时代替，实际应使用 PhoneAuthService
) -> BaseResponse[AuthResponse]:
    """
    手机号验证码登录接口
    ...
    # 实际使用时应该：
    # from app.api.v1.auth.dependencies import PhoneAuth
    # auth_service: PhoneAuth = Depends()
    from app.api.v1.auth.service import PhoneAuthService
